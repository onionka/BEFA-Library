...
#ifndef BFP_PLUGIN_MANAGER_HPP
#define BFP_PLUGIN_MANAGER_HPP
          ...
          void addPluginLibrary(::std::string _plug)
            {
              ...
              for (auto plugin: PluginFactory.back()
                                             ->getPlugins())
                Plugins.push_back(PluginFactory.back()
                                               ->create(plugin));
            }
                ...
                if (::boost::filesystem::is_regular_file(*_plugins))
                  {
                    ...
                    (*this->end())->addPluginLibrary((*_plugins).path()
                                                                .string());
                  }
            }
        };
  }
#endif // BFP_PLUGIN_MANAGER_HPP
